...
export const registerUser = asyncHandler(async (req, res, next) => {
    ...
    const result = await db.insert(userSchema).values({ name, email, password: hashedPassword });
    const insertId = result[0].insertId;
    const [user] = await db.select().from(userSchema).where(eq(userSchema.id, insertId));
    // generate tokens
    const accessToken = tokenService.createAccessToken(insertId);
    const refreshToken = tokenService.createRefreshToken(insertId);
    // save refresh token in database
    await db.insert(refreshTokenSchema).values({ token: refreshToken, userId: insertId });
...
export const loginUser = asyncHandler(async (req, res, next) => {
    ...
    // check if email and password are provided
    if (!email || !password) {
        throw new ErrorHandler(404, "Please provide an email and password.");
    }
    // check if user exists in database
...
export const logoutUser = asyncHandler(async (req, res, next) => {
    ...
    if (!refreshToken) {
        throw new ErrorHandler(400, "Please login first to logout.");
    }
    ...
    if (!foundRefreshToken.user) {
        throw new ErrorHandler(400, "User associated to the refresh token does not exist.");
    }
